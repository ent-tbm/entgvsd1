...
!     TO BE UPDATED AT UMASSB TO USE BNU LAI INSTEAD OF LAI3g.
!     DOWNLOAD MOST RECENT BNU LAI FROM: http://globalchange.bnu.edu.cn/research/lai
!
!     Process LAI, Monfreda, CRU and GPCC climate files into Ent PFT
...
!     8/2015 - Carlo Montes modified for 1 km LAI3g
!     1/31/2018 - N. Kiang edited in comments to update to latest BNU LAI, 8-day.     
...
!***************************************************
     ...
     &     /)
      ...
      !### EDIT ME ###!
      !# To calculate BNU LAIMAX:
      !# To use 8-day BNU LAI, add array DOY(ndates) for day of year of observation, where
      !# ndates is number of dates of observations.
      !# DOY must be character for file names, as fortran does not have a convenient function
      !# for translating from number to character.
!       integer, parameter :: ndates = 46
!       character*3, parameter :: DOY(ndates) =
!     &     (/
...
!     &     'LAI3g_1kmx1km.nc'                            !### EDIT ME ####!
      fileinlai = trim(PathFilepre)//trim(PathFilepost)//
     &     'LAI3gMax_1kmx1km.nc'                          !### EDIT ME ####!
...
!     * 19 - Bare or sparsely vegetated
            ...
            enddo
!     * --------- FINISH LAI LC-WEIGHTED SUM ------------------- *!
!     * Normalize by summed ENTPFTLC(k) -- 2018-01-31 N. Kiang   *
!       do k = 1,ENTPFTNUM
          if (ENTPFTLC(k).gt.0.) then
...
!     write(*,*) err
     ...
     &        'nancy.y.kiang@nasa.gov')                            !### EDIT ME ###!
         err = NF_PUT_ATT_TEXT(entpft_fileid(k),NF_GLOBAL,
     ...
     &         'downscaled from 1/12 degree to 1km resolution')      !### EDIT ME ###!
         err = NF_PUT_ATT_TEXT(entpftlaimax_fileid(k),NF_GLOBAL,
     &        'institution',100, 'Original data:  LAI3g,',           !### EDIT ME ###!
     ...
     &         'downscaled from 1/12 degrees')                       !### EDIT ME ###!
         err = NF_PUT_ATT_TEXT(entpftlaimax_fileid(k),NF_GLOBAL,
     &        'creator_name',100, 'NASA GISS')
         err = NF_PUT_ATT_TEXT(entpftlaimax_fileid(k),NF_GLOBAL,
     &        'creator_email',100, 'carlo.montes@nasa.gov,',
