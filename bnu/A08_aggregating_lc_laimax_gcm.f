...
!---------------------------------------------------------------------------------------------
      ...
      program aggregating_lc_laimax_gcm
...
!***************************************************
     ...
     &     /)
      ...
      integer :: k,p,i,j,m
      real*4 :: layerin(longin,latin),layerinpre(longin,latin)
      ...
      integer :: varidx,varidy,varidout(NUMLAYERS)
...
!     INPUT FILE
	 ...
	 write(*,*) inqvar
         err = NF_INQ_VARID(fileidin,inqvar,varidin(k))
	 write(*,*) filein, inqvar, 'layers in'
	 write(*,*) err, fileidin, varidin(k)
      enddo
! --------------------------------------------------------
!     OUTPUT FILE
      PathFilepre= '../lc_lai_ent/'
      PathFilepost= 'EntMM_lc_laimax_'//res_out//'.nc '
	 ...
	 write(*,*) fileout, inqvar, 'layers out'
	 write(*,*) err, fileidout, varidout(k)
      enddo
...
! --------------------------------------------------------
      ...
      do k = 1,NUMLAYERS
         ...
         !* read layers
         err = NF_GET_VARA_REAL(fileidin,varidin(k),start,
     &        count,layerin)
         write(*,*) err
     ...
     *        longout,latout,OFFIB,DLATB, DATMIS)
         ...
         err = NF_PUT_VARA_REAL(fileidout,varidout(k),start,count,
     &        layerout)
         write(*,*) err
      ...
      end program aggregating_lc_laimax_gcm
