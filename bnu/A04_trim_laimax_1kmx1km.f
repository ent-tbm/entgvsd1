...
!      character*(*), parameter :: res_in_int="1440x720"
      ...
      integer :: varidlaim(KM),varidlcm(KM)
      ...
      integer :: fidsum,varidlcsum,varidlaisum,varidxsum,varidysum
      character*80 :: filebs
      integer :: fileidbs,varidbs
      character*60 :: PathFilepre, PathFilepost

      integer :: fidlaiin(KM),fidlaiout(KM),fidlcin(KM)
      integer :: varidlaiin(KM),varidlaiout(KM),varidlcin(KM)

      character*256 :: fllaimaxpure,fllcmaxpure,flheightpure
      integer :: fidlaimaxpure,fidlcmaxpure,fidlaim(12),fidheightpure
      integer :: varidlaimaxpure(18),varidlcmaxpure(18)
      integer :: varidheightpure(18)
...
!      real*4 hmnc, hsdnc
      ...
      character*256 :: fileinnc,fileoutnc
	 ...
	 PathFilepost=  EntPFT_files(k)
         ...
         err = NF_OPEN(filelcmax(k),0,fidlcin(k))
         write(*,*) err,fidlcin(k)
         inqvarin = EntPFT_lcfields(k)
         err = NF_INQ_VARID(fidlcin(k),inqvarin,varidlcin(k))
         write(*,*) err,fidlcin(k),varidlcin(k),EntPFT_lcfields(k)
      enddo
     ...
     &        '_lai.nc'
         err = NF_OPEN(filelaimax(k),0,fidlaiin(k))
         write(*,*) err,filelaimax(k)
         inqvarin = EntPFT_laifields(k)
         err = NF_INQ_VARID(fidlaiin(k),inqvarin,varidlaiin(k))
         write(*,*) err
      enddo
