...
!     ENTPFTLC
     ...
     &        varidlcin(k))
	 write(*,*) filelcin, inqvarin, 'lc in'
	 write(*,*) err, fileidlcin(k), varidlcin(k)
      enddo

!     WATERLAI
      PathFilepre= '../lc_lai_ent/'
      PathFilepost= 'EntMM_lc_laimax_1kmx1km/'
      filewlai = trim(PathFilepre)//trim(PathFilepost)//
     &     'water_lai.nc'
      err = NF_OPEN(filewlai,NF_WRITE,fileidwlai)
      err = NF_INQ_VARID(fileidwlai,'water_lai',varidwlai)
      write(*,*) 'water lai', err, varidwlai
...
!     ENTPFTLAIMAX
     ...
     &        varidlaimaxin(k))
	 write(*,*) filelaiin, inqvarin, 'lai in'
	 write(*,*) err, fileidlaimaxin(k), varidlaimaxin(k)
      enddo
...
!     ENTPFT LC LAIMAX
      PathFilepre= '../lc_lai_ent/'
      PathFilepost= 'EntMM_lc_laimax_1440x720.nc '
      filelclaiout = trim(PathFilepre)//trim(PathFilepost)
      err = NF_OPEN(filelclaiout,NF_WRITE,fileidlclaiout)
      ...
      do k=1,NUMLAYERS
         ...
         err = NF_INQ_VARID(fileidlclaiout,inqvarin,
     &        varidlclaimaxout(k))
	 write(*,*) inqvarin, ' out'
	 write(*,*) err, varidlclaimaxout(k)
      enddo
      err = NF_INQ_VARID(fileidlclaiout,'lon',
     &     varidlonlclai)
      err = NF_INQ_VARID(fileidlclaiout,'lat',
     &     varidlatlclai)
...
! ------------------------------------------------------------------
      ...
      err = NF_PUT_VARA_REAL(fileidlclaiout,varidlonlclai,1,IM25,lon)
      write(*,*) err, ' lon'
      err = NF_PUT_VARA_REAL(fileidlclaiout,varidlatlclai,1,JM25,lat)
      write(*,*) err, ' lat'
...
!     MAIN LOOP
     ...
     *           longout,latout,OFFIB,DLATB, DATMIS)
            ...
            count(1) = IM25
            count(2) = JM25
            err = NF_PUT_VARA_REAL(fileidlclaiout,varidlclaimaxout(k),
     &           start,count,lcout)
            write(*,*) 'water lcout ', k, err
!	    err = NF_CLOSE(fileidwlc)
...
!	    err = NF_CLOSE(fileidlcin(k-1))
         ...
         elseif (k.eq.21) then
            ...
            err = NF_GET_VARA_REAL(fileidwlai,varidwlai,
     &           start,count,laiin)
     ...
     *           longout,latout,OFFIB,DLATB, DATMIS)
            ...
            do i=1,longin
               do j=1,latin
                  !* Weights, if any
                  WTIN(i,j) = 1.
                  if (lclaiprod(i,j).eq.undef) WTIN(i,j) = 0.
               end do
            end do
     ...
     *           longout,latout,OFFIB,DLATB, DATMIS)
            ...
            do i = 1,IM25
		     ...
		     laimaxout(i,j) = undef
                     ...
                     laimaxout(i,j) = lclaiprodout(i,j)/lcout(i,j)
!		     write(*,*) lclaiprodout(i,j),lcout(i,j),laimaxout(i,j)
                  endif
               enddo
            enddo
	    ...
	    start(1) = 1
            ...
            count(2) = JM25
            err = NF_PUT_VARA_REAL(fileidlclaiout,varidlclaimaxout(k),
     &           start,count,laimaxout)

            write(*,*) 'laimaxout ', k, err
!	    err = NF_CLOSE(fileidwlai)
!	    err = NF_CLOSE(fileidwlc)
         ...
         elseif (k.gt.21 .and. k.le.NUMLAYERS) then
            ...
            err = NF_GET_VARA_REAL(fileidlaimaxin(k-21),
     &           varidlaimaxin(k-21),start,count,laiin)
...
!		     write(*,*) lclaiprodout(i,j),lcout(i,j),laimaxout(i,j)
     ...
     &           start,count,laimaxout)
	    ...
	    err = NF_PUT_VARA_REAL(fileidlclaiout,varidlclaimaxout(k-1),
     &           start,count,laimaxout)
            write(*,*) 'laimaxout ', k, err
!	    err = NF_CLOSE(fileidlaimaxin(k-21))
!	    err = NF_CLOSE(fileidlcin(k-21))
      ...
      end program aggregating_lc_laimax
