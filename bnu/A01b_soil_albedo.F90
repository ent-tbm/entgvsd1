...
!          'NIR3 (1250-1500), NIR4 (1500-2200), NIR5 (2200-4000)'
    ...
    character*5, parameter :: sbright_dark(2) = (/"bright", "dark "/)
    ...
    ! Input files
    type(ChunkIO) :: ioall_lc, io_lcice, io_lcwater
    ...
    call chunkerlr%nc_reuse_var(ioall_lc, io_lcice, (/1,1,ent2%svm(SNOW_ICE)/))
    call chunkerlr%nc_reuse_var(ioall_lc, io_lcwater, (/1,1,ent2%svm(CV_WATER)/))
    ...
    do iband=1,NBANDS_GISS
        call chunker%nc_create(io_albgiss(iband), weighting(wta,1d0,0d0), &
            'carrer/', &
            'albgiss'//trim(sbands_giss(iband)), &
            'albgiss'//trim(sbands_giss(iband)), &
    ...
    ! ----------- fracbd
    do iband=1,NBANDS_GISS
        call chunker%nc_create(io_fracbd(iband), weighting(wta,1d0,0d0), &
            'carrer/', &
            'fracbd'//trim(sbands_giss(iband)), &
            'fracbd'//trim(sbands_giss(iband)), &
    ...
    ! -------------- fracgrey
    call chunker%nc_create(io_fracgrey, weighting(wta,1d0,0d0), &
        'carrer/', &
        'fracgrey', &
        'fracgrey', &
        'Bright/Dark Soil in GISS Bands', '1', &
    ...
    do ichunk = 1,chunker%nchunk(1)
        ...
        do ic = 1,chunker%chunk_size(1)
          ...
          enddo   ! k=1,NBANDS
            ...
            ! to cell's mixed albedo.
            io_fracgrey(BRIGHT)%buf(:,:) = 0
            io_fracgrey(DARK)%buf(:,:) = 0

            if (io_albsw%buf(ic,jc).ne.FillValue) then 
               if ((io_lcice%buf(ic,jc)+io_lcwater%buf(ic,jc)).eq.1.0) then !No ground
                    io_fracgrey(BRIGHT)%buf(:,:) = FillValue
                    io_fracgrey(DARK)%buf(:,:) = FillValue
