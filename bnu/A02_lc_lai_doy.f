...
do k = 1,2
   ...
   filelai = trim(PathFilepre)//trim(PathFilepost)
   err = NF90_OPEN(filelai,NF90_WRITE,laifileid(k))
rite(*,*) err, filelai
   err = NF90_INQ_VARID(laifileid(k),'lai',laivarid(k))
rite(*,*) err
   err = NF90_INQ_VARID(laifileid(k),'lon',varidx(k))
rite(*,*) err
   err = NF90_INQ_VARID(laifileid(k),'lat',varidy(k))
rite(*,*) err
enddo
...
filewater = trim(PathFilepre)//trim(PathFilepost)// &
     'water_lc.nc'
err = NF90_OPEN(filewater,NF90_WRITE,waterfileid)
write(*,*) err
err = NF90_INQ_VARID(waterfileid,'water_lc',watervarid)
write(*,*) err
...
do k = 1,19
   ...
   err = NF90_OPEN(filepft,NF90_WRITE,pftfileid(k))
	 write(*,*) err
   inqvarin = trim(EntPFT_files2(k))
   err = NF90_INQ_VARID(pftfileid(k),inqvarin,pftvarid(k))
rite(*,*) err
enddo
...
do k = 1,2
   fileout = '../lc_lai_ent/nc/EntMM_lc_lai_' &
        //DOY(k)//'_1kmx1km.nc'
   err = NF90_OPEN(fileout,NF90_WRITE,fileidout(k))
rite(*,*) err, fileout
   inqvarin = 'EntPFT'
   err = NF90_INQ_VARID(fileidout(k),inqvarin,varidout(k))
rite(*,*) err
enddo
...
do ycoord = 1,JM1km
   ...
   do xcoord = 1,IM1km
      ...
      if (xcoord.eq.1) then
         startY(1) = ycoord
         countY(1) = 1
         err = NF90_GET_VARA_REAL(laifileid(1),varidy,startY, &
              countY,lat)
      endif
...
!**   LAI data ------------------------------------------------------------------------------
      ...
      do k = 1,2
         ...
         if (ycoord.eq.1) then
            startX(1) = xcoord
            countX(1) = 1
            err = NF90_GET_VARA_REAL(laifileid(k),varidx(k),startX, &
                 countX,lon)
         endif
         ...
         if (ycoord.eq.1) then
            err = NF90_PUT_VARA_REAL(fileidout(k),varidx(k),startX, &
                 countX,lon)
         endif
         if (xcoord.eq.1) then
            err = NF90_PUT_VARA_REAL(fileidout(k),varidy(k),startY, &
                 countY,lat)
         endif
                        
         err = NF90_GET_VARA_REAL(laifileid(k),laivarid(k),startB, &
              countB,LAI)
         ...
         do p = 1,NUMLAYERSLC
            ...
            if (p.eq.1) then
               ...
               err = NF90_GET_VARA_REAL(waterfileid,watervarid, &
                    startB,countB,LCIN_in)
!                     write(*,*) err
               
               lai_lc = LCIN_in*LAI

               err = NF90_PUT_VARA_REAL(fileidout(k),varidout(k), &
...
!                     write(*,*) err
            ...
            if (p.gt.1 .and. p.le.NUMLAYERSLC) then
               ...
               err = NF90_GET_VARA_REAL(pftfileid(p-1), &
