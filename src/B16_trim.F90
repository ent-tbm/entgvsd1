...
!  ModelE.  The following filesets are produced:
!
!   trimmed/: trim out small subgrid fractions, and conserve total
!      grid LAI.
!
...
subroutine outputsegment_open(this, step, esub)
    ...
    ! Open checksum files
    call this%chunker%file_info(info, esub, LAI_SOURCE, 'M', 'lc', LAI_YEAR, step, '1.1', &
        varsuffix = '_checksum')
    ...
    call this%chunker%file_info(info, esub, LAI_SOURCE, 'M', 'lclaimax', LAI_YEAR, step, '1.1', &
        varsuffix = '_checksum')
    ...
    call this%chunker%file_info(info, esub, LAI_SOURCE, 'M', 'lchgt', LAI_YEAR, step, '1.1', &
        varsuffix = '_checksum')
    ...
    do m=1,NMONTH
        call this%chunker%file_info(info, esub, LAI_SOURCE, 'M', 'lclai', LAI_YEAR, step, '1.1', &
...
subroutine do_trim(rw, esub)
    ...
    call chunker_pu%nc_open_set( &
        esub_p, io_ann_hgt, &
        LAI_SOURCE, 'M', 'hgt', LAI_YEAR, 'purelr', '1.1')
    ...
    do m=1,NMONTH
        call chunker_pu%nc_open_set( &
            esub_p, io_mon_lai(:,m), &
            LAI_SOURCE, 'M', 'lai', LAI_YEAR, 'purelr', '1.1', &
            doytype='month', idoy=m)
    end do
...
implicit none
    ...
    type(ReadWrites_t) :: rw
    call rw%init(THIS_OUTPUTS_DIR, 'B16_trim', 1000,1000)
