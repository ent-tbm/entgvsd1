...
!------------------------------------------------------------------------------
     ...
     !Keep local to each program to define own values.
      subroutine my_nf_defglobal(file, Description, Comments)
      use netcdf
      use convertnc
      character*(*) :: file
...
!------------------------------------------------------------------------------
      ...
      real*4, parameter :: undef = -1.e30 !Carrer/Montes original -9999.9. NK version switched it to -1.e30
...
!     &     'NIR3 (1250-1500), NIR4 (1500-2200), NIR5 (2200-4000)'
      ...
      real*4, parameter :: undef_out = -1.e30 !AViewer, GCM     !## UPDATE ME
...
!     &     'Carrer2014_means_2004_Qingsong/'//
     ...
     i        IM,JM,1,IM,1,JM,dimlon,dimlat,
!     o        "soilalb_VIS_"//trim(vark),
     o        trim(vark),
     o        "Carrer soil albedo VIS (300-700 nm) annual "
     o        //trim(vark),
     o        "fraction", varin)
         !status = nf_close(ncidin)
...
!      write(*,*) status, 'nf_open in ',trim(filein)
     ...
     &        ,varid,varin)
         ...
         status = my_nf_inq_def_put_var_real32_2(ncidout,
     i        IM,JM,1,IM,1,JM,dimlon,dimlat,
!     o        "soilalb_NIR_"//trim(vark),
     o        trim(vark),
     o        "Carrer soil albedo NIR (700-5000 nm) annual "
     o        //trim(vark),
     o        "fraction", varin)
      enddo
      status = nf_close(ncidin)
...
!      !  For partial-land cells (that have ice and/or water) use
       ...
       do i=I0o,I1o
         do j=J0o,J1o
            do k=1,N_BANDS
               if (albgiss(i,j,k).ne.undef_out) then
     ...
     &                   ((lc_ice(i,j)+lc_water(i,j)).lt.1.0)) then  !Partial
                     ...
                     enddo
                     if (s.ne.undef_out) exit
                    enddo
                    if (s.ne.undef_out) exit
                   enddo
                   if (s.eq.undef_out) then !No nearby ground cells found 
                      !fracbd(i,j,k,BRIGHT) = undef_out
                      !fracbd(i,j,k,DARK) = undef_out
