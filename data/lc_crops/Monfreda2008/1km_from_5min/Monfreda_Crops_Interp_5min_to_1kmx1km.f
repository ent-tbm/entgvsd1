!* Monfreda_Crops_Interp_5min_to_1kmx1km.f - Specifically set up to downscale 0.5º Monfreda crops to 1km
!* Author: Carlo Montes, Nancy Y. Kiang
...
!* To compile on gibbs:
!* gfortran  -cpp -fconvert=big-endian -O0 -fno-range-check -I$NETCDFHOME/include -c arrayutil.f  Monfreda_Crops_Interp_5min_to_1kmx1km.f
!* gfortran -o myExe arrayutil.o Monfreda_Crops_Interp_5min_to_1kmx1km.o  -L$NETCDFHOME/lib -lnetcdf
...
C****
      Subroutine HNTR40_OLD (IMA,JMA,OFFIA,DLATA,
...
!------------------------------------------------------------------------
      ...
      character(8) :: date
           ...
           !Enter define mode
      status=nf90_open(trim(file), NF_WRITE, ncid)
     ...
     &     '(Ent GVSD) v1. '
      ...
      status=nf90_put_att_text(ncid, NF_GLOBAL, 'Description'
     &     ,len(trim(text)), trim(text))
      call handle_nf_error(status, 'nf_global'//trim(text))
      text = 'Monfreda et al. (2008) crop multi-harvest cover '//
     &     'normalized to crop type fraction of cover cover '
      !write(0,*) 'len, text: ',len(trim(text)), trim(text)
      status=nf90_put_att_text(ncid, NF_GLOBAL, 'Comments'
     &     ,len(trim(text)), trim(text))
      call handle_nf_error(status, '')
      text = 'Institution:  NASA Goddard Institute for Space Studies'
      status=nf90_put_att_text(ncid, NF_GLOBAL, 'Institution'
     &     ,len(trim(text)), trim(text))
      call handle_nf_error(status, '')
      text = 'Nancy.Y.Kiang@nasa.gov'
      status=nf90_put_att_text(ncid, NF_GLOBAL, 'Contact'
     &     ,len(trim(text)), trim(text))
      call handle_nf_error(status, '')
      call DATE_AND_TIME(date)
      text = date//' Created'
      status=nf90_put_att_text(ncid, NF_GLOBAL, 'History'
     &     ,len(trim(text)), trim(text))
...
!------------------------------------------------------------------------
      ...
      character*8, parameter :: pathin='../5min/'

      !*      OUTPUT Crop netcdf files                 *
     ...
     &     /)
      ...
      integer, parameter :: IM5M = 4320 !long at 5 minutes ~ 10 km
      integer, parameter :: JM5M = 2160 !lat at 5 minutes ~ 10 km
...
!     DLATA = NINT(360./latin) * 60.
     ...
     *     longout,latout,OFFIB,DLATB, DATMIS)
      ...
      !* Input file.
      filein = pathin//'Monfreda_crops_5min_norm.bin'
	...
	write(*,*) fileoutnc
        ...
        call my_nf_defglobal(trim(fileoutnc))
