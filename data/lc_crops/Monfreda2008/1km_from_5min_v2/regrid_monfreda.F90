...
subroutine regrid_control(rw, root, dir, leaf, vnames)
    ...
    call chunker_out%init(im1km,jm1km,imh,jmh,'forplot',4,30,30, nchunk=(/1,5/), outputs_dir=THIS_OUTPUTS_DIR)
    ...
    call chunker_out%nc_create1(ioall_out, weighting(chunker_out%wta1,1d0,0d0), &
        './', trim(leaf)//'_1km', info)!, vname, vname, '1', create_lr=.true.)
    ...
    call chunker_in%init(IM5m,JM5m,  IMh,JMh, 'forplot', 30, 30, 30, (/1,5/), outputs_dir=THIS_OUTPUTS_DIR)
    do i=1,nvnames
        call chunker_in%nc_open(io_in(i), root, dir, trim(leaf)//'_5min.nc', vnames(i), 1)
    end do
...
implicit none
    ...
    call regrid_control(rw, '../', '5min/', 'Monfreda_crops', &
        (/ 'c3crop', 'c4crop' /) )
