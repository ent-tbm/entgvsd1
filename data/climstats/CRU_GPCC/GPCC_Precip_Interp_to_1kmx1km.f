!* GPCC_Precip_Interp_to_1kmx1km.f - Specifically set up to interpolate 0.5º CRU temp files
...
!* gfortran -c -cpp -mcmodel=medium -fconvert=big-endian -O2 -fno-range-check -I/usr/local/other/SLES11.3/netcdf/3.6.3/gcc-4.9.2-sp3/include GPCC_Precip_Interp_to_1kmx1km.f
!* gfortran -o myExe arrayutil.o GPCC_Precip_Interp_to_1kmx1km.o -L/usr/local/other/SLES11.3/netcdf/3.6.3/gcc-4.9.2-sp3/lib -lnetcdf

!* In this case, the executable is called "myExe"

!!!!  CHECK LAND MASK FOR COASTLINES AND ASSIGN WTIN VALUES.
!  g95 -fno-second-underscore -O2 -fendian=big -cpp hntr4_2x2.5.f
! gfortran -cpp -fconvert=big-endian -O2 -fno-range-check arrayutil.f GPCC_Precip_Interp_to_1kmx1km.f -I/opt/local/include -L/opt/local/lib -lnetcdff -lnetcdf
...
!------------------------------------------------------------------------
      ...
      program GPCC_Precip_Interp_to_1kmx1km
...
!      real*4, parameter :: undef_in = -1.e30
      ...
      real*4 :: prec(time,longout,latout)
...
!     DLATA = NINT(360./latin) * 60.
     ...
     *     longout,latout,OFFIB,DLATB, DATMIS)
      ...
      !* Input file.
      filein =  'GPCC_v6_PREC_means_1981-2010_HXH_bis.nc'
!      filein =  'GPCC_v6_PREC_means_1981-2010_10kmx10km.nc'
      status = nf_open(trim(filein),NF_WRITE,ncidin)
      write(*,*) status, 'nf_open in ',trim(filein)

      !* Output file.
      fileout = 'GPCC_v6_PREC_means_1981-2010_1kmx1km.nc'
      ...
      do k=1,13
        ...
        !TITLE = 'TEST MAX LAI3G'
        !LAYERIN(1,:) = 1.0
        !write(99) TITLE, LAYERIN
...
!     Make and put lon and lat
      ...
      end program GPCC_Precip_Interp_to_1kmx1km
