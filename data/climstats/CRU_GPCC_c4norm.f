! from sheffield_c4norm.f - Calculate normalized C4 climate.  No rescaling.
! Modified by Carlo Montes: August 10, 2014
!        Climate classification based on gridded CRU temperature and GPCC precipitation
!        Data exported as netcdf also
!  g95 -fno-second-underscore -O2 -fendian=big -cpp hntr4_sheffield.f
!  gfortran -cpp -fconvert=big-endian -O0 -fno-range-check arrayutil.f CRU_GPCC_c4norm.f -I/opt/local/include -L/opt/local/lib -lnetcdff -lnetcdf
...
!------------------------------------------------------------------------
      program CRU_GPCC_c4norm
       ! Read in Sheffield 1x1 degree file, 
       ! get C4 climate and calculate C4norm.
      ...
      real*4, parameter :: undef_out = -1e-30 !AViewer undef value
     ...
     &     /)
      ...
      character*256 :: filein, fileout, fileoutnc
      real*4 :: LAYERIN(longin,latin)
      real*4 :: LAYEROUT(longout,latout)
      real*4 :: WTIN(longin,latin)
      real*4 :: OFFIA, DLATA, OFFIB, DLATB, DATMIS
      real*4 :: C4climate(longin,latin)
      real*4 :: varout(longout,latout)
      real*4 :: lon(longout),lat(latout)
      character*20 :: inqvarin, inqvarout
      integer :: i, j, k, f, m, ncidin, ncidout, varid, status


      !----------------------------------------------------------------
      !* C4norm scale
      !* Input file.
      filein = 'CRU_GPCC_climstats_1981-2010_1kmx1km.bin' !1x1
      write(*,*) filein
      open(10,file=filein,form='unformatted',status='old')
      !* Normalize Sheffield stats to 0-1 for #months 1-7, and 1 for 7+ months.
      !* I.e. If >7 months/year have C4 climate, then 100% C4, else scale.
      do i=1,3
         read(10) !skip
      enddo
      read(10) TITLE,C4climate
      do i=1,longin
         do j=1,latin
            if (C4climate(i,j).le.7.) then
               C4climate(i,j) = C4climate(i,j)/7.
            else
               C4climate(i,j) = 1.0
            endif
         enddo
      ...
      !* Output file
      fileout = 'CRU_GPCC_C4norm_1981-2010_1kmx1km.bin'
      open(50,file=fileout,form='unformatted')
      TITLE = 'C4 climate (herb fraction that is C4) Collatz months'
      write(50) TITLE, C4climate
      close(50)
      close(10)

      !----------------------------------------------------------------
      !----------------------------------------------------------------
      !* Interpolate C4climate.  ! JUNK - NO OUTPUT FROM HERE
      !DIVJ = 360. / NINT(360./latout)
      !DLATA = NINT(360./latin) * 60.
      DLATA = (180./latin) * 60.
      OFFIA=0.0
      !DLATB = NINT(180./latout) * 60.
      DLATB = (180./latout) * 60.  !Remove NINT for 0.5 degrees
      OFFIB=0.0
      DATMIS=-1.e30
     ...
     *     longout,latout,OFFIB,DLATB, DATMIS)
      write(*,*) 'Got to after HNTR40.'

      LAYEROUT(:,:) = 0.

      !* Output file.
      !fileout = 'Climstats/Sheff1951-2006_C4norm_05x05.bin'
      write(*,*) fileout
...
!        end do
      ...
      !AViewer undef
      !if (LAYERIN(i,j).eq.undef_in) LAYERIN(i,j)=undef_out 
      !* Weights, if any
      !WTIN(i,j) = 1.            !FGRNDIN
      !!if (LAYERIN(i,j).le.undef_out) WTIN(i,j) = 0.

      !call HNTR4P(WTIN, C4climate, LAYEROUT)

      !TITLE = 'C4 climate (herb fraction) Collatz 0.5 x 0.5 deg'
      ...
      !* Output file .nc
      fileoutnc = 'CRU_GPCC_C4norm_1981-2010_1kmx1km.nc'
